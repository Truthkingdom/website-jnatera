...
if ( ! function_exists( 'az_normal_pagination' ) ) {
	...
	function az_normal_pagination( $query = null ) {  
		...
		if( $pages > 1 ) {
			...
			if ( $page + 1 <= $pages ) {
				$output .= '<div class="prev-post"><a href="' . get_pagenum_link( $page + 1 ) . '">' . '<div class="pagination-inner">' . __( 'Older Post' , AZ_THEME_NAME ) . '</div><span class="icon-arrow-pag font-icon-arrow-left-simple-thin-round"></span></a></div>';
			}
			if ( $page - 1 >= 1 ) {
				$output .= '<div class="next-post"><a href="' . get_pagenum_link( $page - 1 ) . '"><div class="pagination-inner">' . __( 'Newer Post' , AZ_THEME_NAME ) . '</div><span class="icon-arrow-pag font-icon-arrow-right-simple-thin-round"></span></a></div>';
			}
			$output .= '</div>';
    ...
    function az_custom_get_page_title() {
        ...
        if( is_archive() ) {
                if( is_category() ) {
                    ...
                    $page_title = __('All Posts By &#8220;'.$curauth->display_name.'&#8221;', AZ_THEME_NAME);
                } 
            } 
		elseif( is_search() ) {
		...
		if ( ! empty( $excluded_categories ) ) {
			if ( ! is_array( $excluded_categories ) ) {
				if ( false !== strpos( $excluded_categories, ' and ' ) ) { 
					_deprecated_argument( __FUNCTION__, '3.3', sprintf( __( 'Use commas instead of %s to separate excluded terms.', AZ_THEME_NAME ), "'and'" ) ); 
...
require_once($tempdir .'/framework/redux/redux-core/framework.php');
...
$license_key = $options_alice['envato-license-key'];
...
if ( $license_key && $options_alice['enable-auto-updates'] == 1 ) {
	new WPUpdatesThemeUpdater_1277( 'http://wp-updates.com/api/2/theme', basename( get_template_directory() ), $license_key  );
}
