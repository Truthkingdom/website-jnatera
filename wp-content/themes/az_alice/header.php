...
<meta name="apple-mobile-web-app-title" content="<?php echo __( $bookmark_title, AZ_THEME_NAME); ?>">
<?php } ?>
...
<body <?php body_class($device_version); ?>>
...
<div class="modal-container-share">
    ...
    <div class="modal-share">
        ...
        <a class="az-share-twitter share-btn-footer" href="https://twitter.com/home?status=<?php echo !empty( $title ) ? $title : ''; ?>+<?php echo !empty( $url ) ? $url : ''; ?>" title="<?php _e( 'Share on Twitter', AZ_THEME_NAME ); ?>" target="_blank"><i class="font-icon-social-twitter"></i><span>Share</span></a>
        <?php } ?>
        <?php if( !empty($options_alice['header_facebook_share']) && $options_alice['header_facebook_share'] == 1) { ?>
        <a class="az-share-facebook share-btn-footer" href="https://www.facebook.com/share.php?u=<?php echo !empty( $url ) ? $url : ''; ?>&amp;title=<?php echo !empty( $title ) ? $title : ''; ?>" title="<?php _e( 'Share on Facebook', AZ_THEME_NAME ); ?>" target="_blank"><i class="font-icon-social-facebook"></i><span>Share</span></a>
        <?php } ?>
        <?php if( !empty($options_alice['header_google_share']) && $options_alice['header_google_share'] == 1) { ?>
        <a class="az-share-google share-btn-footer" href="https://plus.google.com/share?url=<?php echo !empty( $url ) ? $url : ''; ?>" title="<?php _e( 'Share on Google Plus', AZ_THEME_NAME ); ?>" target="_blank"><i class="font-icon-social-google-plus"></i><span>Share</span></a>
        <?php } ?>
        <?php if( !empty($options_alice['header_pinterest_share']) && $options_alice['header_pinterest_share'] == 1) { ?>
        <a class="az-share-pinterest share-btn-footer" href="http://pinterest.com/pin/create/bookmarklet/?media=<?php echo !empty( $media[0] ) ? $media[0] : ''; ?>&amp;url=<?php echo !empty( $url ) ? $url : ''; ?>&amp;is_video=false&amp;description=<?php echo !empty( $title ) ? $title : ''; ?>" title="<?php _e( 'Share on Pinterest', AZ_THEME_NAME ); ?>" target="_blank"><i class="font-icon-social-pinterest"></i><span>Share</span></a>
        <?php } ?>
        <?php if( !empty($options_alice['header_linkedin_share']) && $options_alice['header_linkedin_share'] == 1) { ?>
        <a class="az-share-linkedin share-btn-footer" href="http://www.linkedin.com/shareArticle?mini=true&amp;url=<?php echo !empty( $url ) ? $url : ''; ?>&amp;title=<?php echo !empty( $title ) ? $title : ''; ?>" title="<?php _e( 'Share on Linkedin', AZ_THEME_NAME ); ?>" target="_blank"><i class="font-icon-social-linkedin"></i><span>Share</span></a>
        <?php } ?>
    </div>
</div>
<!-- End Modal Share -->
    ...
    <div class="modal-search">
        <div class="modal-search-wrap">
            <form method="get" id="searchform" action="<?php echo home_url(); ?>/">
                <input id="search-modal" type="text" name="s" value="" autocomplete="off" placeholder="<?php _e('Search', AZ_THEME_NAME); ?>" />
            </form>
            <h4 class="search-subtitle"><?php _e('Type here &amp; click enter', AZ_THEME_NAME); ?></h4>
        </div>
    </div>
</div>
<!-- End Modal Search -->
