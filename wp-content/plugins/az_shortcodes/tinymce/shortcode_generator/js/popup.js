jQuery(document).ready(function($){
    ...
    function dynamic_items(){
		...
		var accordionContent, toggleContent, tabContent, testimonialContent;
		...
		//testimonials
		if( $('.shortcode-options[data-name=az_testimonial_section]').is(':visible') ) {
			$('.shortcode-options[data-name=az_testimonial_section] .shortcode-dynamic-item').each(function(){		
				testimonialImage = $(this).find('.redux-opts-screenshot:first').attr('src');
				testimonialContent = ($(this).find('.shortcode-dynamic-item-text').val().length != 0) ? $(this).find('.shortcode-dynamic-item-text').val() : 'none';
				code += ' [testimonial image="'+testimonialImage+'" title="'+$(this).find('.shortcode-dynamic-item-input').val()+'"]'+testimonialContent+'[/testimonial]'; 
				testimonialID++;
			});
		}
    ...
    function update_shortcode(ending){
		...
		$('#options-'+name+' input.popup-colorpicker').each(function(){
			 code += ' custom_color="'+ $(this).val()+'"'; 
		});
		
		if(name == 'az_button_sh' && $('.icon-option i.selected').length > 0 ) code += ' icons="'+ $('.icon-option i.selected').attr('class').split(' ')[0] +'"'; 
		
		if(name == 'az_icon_sh' && $('.icon-option i.selected').length > 0 ) code += ' icons="'+ $('.icon-option i.selected').attr('class').split(' ')[0] +'"'; 
	...
	function resetFileds(){
		...
		$('#shortcode-generator').find('select:not(#az-shortcodes) option:first-child').attr("selected", "selected");
		...
		$('#shortcode-generator').find('#shortcode-content').hide();
